...
- (id) initWithURL:(NSURL *)theURL delegate:(id<URLCacheConnectionDelegate>)theDelegate
{
	if (self = [super init]) {
		...
		NSURLRequest *theRequest = [NSURLRequest requestWithURL:theURL
													cachePolicy:NSURLRequestReloadIgnoringLocalCacheData timeoutInterval:6000];
...
- (void) connection:(NSURLConnection *)connection didReceiveData:(NSData *)data
{
	...
	//iTestAppDelegate *appDelegate = (iTestAppDelegate *)[[UIApplication sharedApplication] delegate];
	//[appDelegate.m_pData.m_pReceivedData addObject:data];
}
